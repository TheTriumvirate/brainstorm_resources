...
void main() {
    ...
    color = vec4(data.xyz + (delta.xyz * speed), 1.0);
    if(length(color.rgb - data.rgb) <= reset_threshold) {
        vec4 spawn_noise = texture(uNoise, v_texpos);
        color = spawn_noise;
        if(length(spawn_noise.rgb - data.rgb) <= reset_threshold || data.r <= reset_threshold || data.g <= reset_threshold || data.b <= reset_threshold) {
